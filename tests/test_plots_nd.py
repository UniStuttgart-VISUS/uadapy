...
def test_plot_samples_nd(sample_distributions):
    ...
    fig, axs = plots_nd.plot_samples(sample_distributions, n_samples=100)
    return fig

@pytest.mark.mpl_image_compare(baseline_dir="baseline")
def test_plot_contour_nd(sample_distributions):
    """Test plot_contour function."""
    fig, axs = plots_nd.plot_contour(sample_distributions,n_samples_kde=1000)
    return fig

@pytest.mark.mpl_image_compare(baseline_dir="baseline")
def test_plot_contour_samples_nd(sample_distributions):
    """Test plot_contour_samples function."""
    fig, axs = plots_nd.plot_contour_samples(sample_distributions, n_samples=100, n_samples_kde=1000)
    return fig